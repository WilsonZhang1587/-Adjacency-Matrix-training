...
const data = [
  { id: "1", specs: ["紫色", "套餐一", "64G", "尺寸二"] },
  { id: "2", specs: ["紫色", "套餐一", "128G", "尺寸一"] },
  { id: "3", specs: ["紫色", "套餐二", "128G", "尺寸二"] },
  { id: "4", specs: ["黑色", "套餐三", "256G", "尺寸一"] },
  { id: "5", specs: ["黑色", "套餐一", "128G", "尺寸二"] }
];
const commoditySpecs = [
  { title: "颜色", list: ["红色", "紫色", "白色", "黑色"] },
  { title: "套餐", list: ["套餐一", "套餐二", "套餐三", "套餐四"] },
  { title: "内存", list: ["64G", "128G", "256G"] },
  { title: "種類", list: ["尺寸一", "尺寸二", "尺寸三"] }
];

let array = Array.from({ length: commoditySpecs.length });
const shopAdjoin = new ShopAdjoin(commoditySpecs, data);
const optionSpecs = shopAdjoin.querySpecsOptions(array);
function handleClick(bool, text, index) {
  ...
  commoditySpecs.map(
    (data, index) => `<div>
        ...
        <div style="display:flex;">
            ${data.list.map(item => {
              let bool = optionSpecs.indexOf(item) > -1;
              return `<div class=${bool &&
                "option"} onclick="handleClick(${bool}, ${item}, ${index});">${item}</div>`;
            })}
        </div>
    </div>`
  )
